...
public class DCOSCliSpec extends BaseSpec {
    ...
    public void setupSSHConnections(String user, String password, String dcosCLINode, String clusterLeader) throws Exception {
        ...
        // Setup ssh in cluster leader
        String command = "sshpass -p '" + password + "' ssh-copy-id -i /root/.ssh/id_rsa.pub " + user + "@" +
                clusterLeader + "";
        ...
        for (String node : nodesArray) {
            // rsync public key
            command = "sshpass -p '" + password + "' ssh-copy-id -i /root/.ssh/id_rsa.pub " + user + "@" + node + "";
            localConnection.runCommand(command);
        }
